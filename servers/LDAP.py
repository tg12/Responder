...
def ParseSearch(data):
    if re.search(r'(objectClass)', data):
        ...
        return str(LDAPSearchSupportedCapabilitiesPacket(
            MessageIDASNStr=data[8:9], MessageIDASN2Str=data[8:9]))
    elif re.search(r'(?i)(objectClass0*.*supportedSASLMechanisms)', data):
        return str(LDAPSearchSupportedMechanismsPacket(
            MessageIDASNStr=data[8:9], MessageIDASN2Str=data[8:9]))
...
def ParseNTLM(data, client, Challenge):
    if re.search('(NTLMSSP\x00\x01\x00\x00\x00)', data):
        NTLMChall = LDAPNTLMChallenge(
            MessageIDASNStr=data[8:9], NTLMSSPNtServerChallenge=Challenge)
        NTLMChall.calculate()
...
def ParseLDAPPacket(data, client, Challenge):
    if data[1:2] == '\x84':
        ...
        if Operation == "\x60":
            ...
            if AuthHeaderType == "\x80":
                PassLen = struct.unpack(
                    '<b', data[20 + UserDomainLen + 1:20 + UserDomainLen + 2])[0]
                Password = data[20 + UserDomainLen +
                                2:20 + UserDomainLen + 2 + PassLen]
                SaveToDb({
                    'module': 'LDAP',
                    'type': 'Cleartext',
                    'client': client,
    ...
    if data[5:6] == '\x60':
        ...
        PassLen = struct.unpack(
            "<b", data[12 + UserLen + 1:12 + UserLen + 2])[0]
