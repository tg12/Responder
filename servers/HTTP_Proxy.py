...
def InjectData(data, client, req_uri):
    ...
    if settings.Config.Serve_Exe and req_uri.endswith('.exe'):
        return RespondWithFile(
            client,
            settings.Config.Exe_Filename,
    ...
    if len(data.split('\r\n\r\n')) > 1:
        ...
        if "content-type: text/html" in Headers.lower():
            ...
            if HasBody and len(
                    settings.Config.HtmlToInject) > 2 and not req_uri.endswith('.js'):
                if settings.Config.Verbose:
                    print text(
                        "[PROXY] Injecting into HTTP Response: %s" %
                        color(
                            settings.Config.HtmlToInject, 3, 1))

                Content = Content.replace(
                    HasBody[0], '%s\n%s' %
                    (HasBody[0], settings.Config.HtmlToInject))

        if "content-encoding: gzip" in Headers.lower():
            Content = zlib.compress(Content)

        Headers = Headers.replace(
            "Content-Length: " + Len, "Content-Length: " + str(len(Content)))
        data = Headers + '\r\n\r\n' + Content
    else:
        if settings.Config.Verbose:
...
class HTTP_Proxy(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def handle(self):
        ...
        if settings.Config.Verbose:
            print text(
                "[PROXY] Received connection from %s" %
    ...
    def socket_proxy(self, af, fam):
        ...
        Proxy = Proxy.rstrip('/').replace('http://',
                                          '').replace('https://', '')
        Proxy = Proxy.split(':')
            ...
            if self._connect_to(netloc, soc):
                ...
                if settings.Config.Verbose:
                    print text(
                        "[PROXY] Client        : %s" %
                        color(
                            self.client_address[0], 3))
                    print text(
                        "[PROXY] Requested URL : %s" %
                        color(
                            self.path, 3))
                ...
                for i in ins:
                    if i is soc:
                        ...
                        try:
                            ...
                            if len(data) > 1:
                                data = InjectData(
                                    data, self.client_address[0], self.path)
                        except BaseException:
                            pass
                    else:
                        out = soc
