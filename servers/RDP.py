...
def ParseNTLMHash(data, client, Challenge):  # Parse NTLMSSP v1/v2
    ...
    if NthashLen == 24:
        NTLMHash = SSPIString[NthashOffset:NthashOffset +
        ...
        WriteHash = '%s::%s:%s:%s:%s' % (
            Username, Domain, LMHash, NTLMHash, Challenge.encode('hex'))
    ...
    if NthashLen > 60:
        ...
        Username = SSPIString[UserOffset:UserOffset +
                              UserLen].decode('UTF-16LE')
        WriteHash = '%s::%s:%s:%s:%s' % (
            Username, Domain, Challenge.encode('hex'), NTLMHash[:32], NTLMHash[32:])
...
class RDP(BaseRequestHandler):
    def handle(self):
        try:
            ...
            # Sometimes a client sends a routing cookie; we don't want to miss
            # that let's look at it the other way around..
