...
def get_user_hashes(user_key, hbootkey):
    ...
    enc_nt_hash = V[hash_offset +
                    (24 if lm_exists else 8):hash_offset +
                    (24 if lm_exists else 8) +
                    16] if nt_exists else ""
...
def dump_hashes(Key, samaddr):
    ...
    for user in get_user_keys(samaddr):
        ...
        if not nthash:
            nthash = empty_nt
        hashes = "%s:%d:%s:%s:::" % (get_user_name(user), int(
            user.Name, 16), lmhash.encode('hex'), nthash.encode('hex'))
        hashesdump = "%s:%d:%s:%s:::\n" % (get_user_name(user), int(
            user.Name, 16), lmhash.encode('hex'), nthash.encode('hex'))
        print hashes
        Output += hashesdump
    return Output
