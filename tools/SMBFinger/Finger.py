...
class SMBNegoDataLanMan(Packet):
    ...
    def calculate(self):
        CalculateBCC = str(self.fields["BuffType"]) + \
            str(self.fields["Dialect"])
    ...
    try:
        ...
        if length > 255:
            OsVersion, ClientVersion = tuple(
                [e.replace('\x00', '') for e in data[48 + length:].split('\x00\x00\x00')[:2]])
...
def GetHostnameAndDomainName(data):
    try:
        ...
        # If max length domain name, there won't be a \x00\x00\x00 delineator
        # to split on
        if Hostname == '':
...
def ShowResults(Host):
    ...
    try:
        ...
        enabled = color(
            "SMB signing is mandatory. Choose another target", 1, 1)
        ...
        print color(
            "Hostname: '%s'\nPart of the '%s' domain" %
            (Hostname, DomainJoined), 8, 3)
    except BaseException:
        pass
...
def ShowSmallResults(Host):
    ...
    try:
        ...
        Message = color(
            ...
            (OsVer, DomainJoined, Signing), 4, 0)
        return Message
    except BaseException:
        pass
...
def ShowScanSmallResults(Host):
    ...
    try:
        Hostname, DomainJoined = DomainGrab(Host)
        Signing, OsVer, LanManClient = SmbFinger(Host)
        Message = "['%s', Os:'%s', Domain:'%s', Signing:'%s']" % (
            Host[0], OsVer, DomainJoined, Signing)
        print Message
    except BaseException:
        pass
        ...
        for host in (dtoa(net + n) for n in range(0, 1 << 32 - mask)):
            if CurrentIP == host:
                ...
                p = multiprocessing.Process(
                    target=ShowScanSmallResults, args=(
                        (host, 445),))
                threads.append(p)
                p.start()
        sleep(1)
