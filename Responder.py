...
parser.add_option(
    ...
    '--ip',
    action="store",
    help="Local IP to use \033[1m\033[31m(only for OSX)\033[0m",
    dest="OURIP",
    metavar="10.0.0.21",
    default=None)
...
parser.add_option(
    ...
    "--externalip",
    action="store",
    help="Poison all requests with another IP address than Responder's one.",
    dest="ExternalIP",
    metavar="10.0.0.22",
    default=None)
...
parser.add_option(
    ...
    '--basic',
    action="store_true",
    help="Return a Basic HTTP authentication. Default: NTLM",
    dest="Basic",
    default=False)
parser.add_option(
    '-r',
...
parser.add_option(
    ...
    '--wpad',
    action="store_true",
    help="Start the WPAD rogue proxy server. Default value is False",
    dest="WPAD_On_Off",
    default=False)
parser.add_option(
    '-u',
            ...
            try:
                if settings.Config.Bind_To_ALL:
                    ...
                    self.socket.setsockopt(
                        socket.SOL_SOCKET, 25, settings.Config.Interface + '\0')
...
def serve_thread_udp_broadcast(host, port, handler):
    try:
        ...
        print(color("[!] ", 1, 1) + "Error starting UDP server on port " + \
            str(port) + ", check permissions or other servers running.")
...
def serve_thread_tcp(host, port, handler):
    try:
        ...
        print(color("[!] ", 1, 1) + "Error starting TCP server on port " + \
            str(port) + ", check permissions or other servers running.")
    ...
    try:
        ...
        if OsInterfaceIsSupported():
            ...
            server.socket = ssl.wrap_socket(
                server.socket, certfile=cert, keyfile=key, server_side=True)
            server.serve_forever()
        else:
            server = ThreadingTCPServer((host, port), handler)
            server.socket = ssl.wrap_socket(
        ...
        print(color("[!] ", 1, 1) + "Error starting SSL server on port " + \
            str(port) + ", check permissions or other servers running.")
